...
export default function Home() {
  ...
  const loadLearning = async () => {
    ...
    setRecords(payload.records.map((record) => ({ ...record, diagnosticResponses: responsesByRecord.get(record.id) ?? [] })));
  };
...
function StudentView(props: {
  ...
  const progressUnitOrder = [...new Set(questions.filter((question) => question.grade === 6 && String(question.semester) === progressSemester && question.isPlayable).map((question) => question.unit))];
  const progressQuestions = questions.filter((question) => question.grade === 6 && String(question.semester) === progressSemester && question.isPlayable);
  const progressUnits = progressUnitOrder.filter((unit) => progressQuestions.some((question) => question.unit === unit));
  const progressLessons = [...new Set(progressQuestions.filter((question) => !progressUnit || question.unit === progressUnit).map((question) => question.lesson))];
  ...
  return <main className="app-shell student-shell">
    ...
    <div className="student-layout"><aside className="side-nav" aria-label="학생 메뉴"><button className={tab === "home" ? "active" : ""} onClick={() => progressNav("home")}>홈</button><button className={tab === "questions" ? "active" : ""} onClick={() => progressNav("questions")}>틀린 문항 선택</button><button className={tab === "notebook" ? "active" : ""} onClick={() => progressNav("notebook")}>나의 오답노트</button><button className={tab === "progress" ? "active" : ""} onClick={() => progressNav("progress")}>나의 진도</button><button className={tab === "password" ? "active" : ""} onClick={() => progressNav("password")}>비밀번호 변경</button></aside>
      <section className="student-content">
        ...
        {tab === "progress" && <><div className="page-heading compact"><p className="eyebrow">MY PROGRESS</p><h1>나의<br /><span>완성 현황</span></h1><p>학기와 단원을 고르면 해당 차시의 100점 완료 상태를 확인할 수 있어요.</p></div><div className="progress-board panel"><div className="progress-filters"><label>학기<select value={progressSemester} onChange={(event) => { setProgressSemester(event.target.value); setProgressUnit(""); }}><option value="1">1학기</option><option value="2">2학기</option></select></label><label>단원<select value={progressUnit} onChange={(event) => setProgressUnit(event.target.value)}><option value="">단원을 선택하세요</option>{progressUnits.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select></label></div><div className="progress-summary"><strong>{progressQuestions.filter((question) => records.some((record) => record.questionId === question.id && record.status === "COMPLETED")).length}</strong><span>/ {progressQuestions.length}문항 100점 완료</span></div><div className="progress-lesson-list">{progressLessons.filter((lesson) => !progressUnit || progressQuestions.some((question) => question.lesson === lesson && question.unit === progressUnit)).map((lesson) => { const lessonQuestions = progressQuestions.filter((question) => question.lesson === lesson && (!progressUnit || question.unit === progressUnit)); const completed = lessonQuestions.length > 0 && lessonQuestions.every((question) => records.some((record) => record.questionId === question.id && record.status === "COMPLETED")); return <label className="progress-row" key={lesson}><div><strong>{lesson}</strong><small>{lessonQuestions.length}문항</small></div><input className="progress-check" type="checkbox" checked={completed} disabled aria-label={lesson + " 100점 완료"} /></label>; })}</div></div></>}
