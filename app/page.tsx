...
export default function Home() {
  ...
  const completeOfflineChallenge = async () => {
    if (!activeRecord || !currentQuestion) return;
    try {
      await json<{ record: LearningRecord }>("/api/learning", { method: "POST", body: JSON.stringify({ action: "offline-complete", recordId: activeRecord.id }) });
      await toggleLessonCompletion(currentQuestion.semester, currentQuestion.unit, currentQuestion.lesson, true);
      setActiveRecord(null); setRetryResult(null); setRetryAnswer(""); setActiveConcept(""); setActiveExample(""); setStudentTab("progress"); setNotice("");
      await loadLearning();
    } catch (error) { setNotice((error as Error).message); }
  };
...
function StudentView(props: {
  ...
  if (activeRecord && retryResult === "challenge") return <OfflineChallengeView student={student} question={currentQuestion} onComplete={completeOfflineChallenge} onNotebook={() => { setActiveRecord(null); setRetryResult(null); setChatMessages([]); setTab("notebook"); }} onTeacherHelp={requestTeacherHelp} onLogout={onLogout} />;
...
function OfflineChallengeView({ student, question, onComplete, onNotebook, onTeacherHelp, onLogout }: { student: Student | null; question: Question | null; onComplete: () => void | Promise<void>; onNotebook: () => void; onTeacherHelp: () => void; onLogout: () => void }) {
  return <main className="app-shell student-shell"><header className="student-header"><button className="brand-inline" onClick={onNotebook}><span className="brand-mark">100</span><strong>백점수익</strong></button><div className="student-header-info"><span>{student?.studentNumber}번 학생</span><button onClick={onLogout}>로그아웃</button></div></header><section className="student-content challenge-content"><div className="study-top"><span className="status-badge status-orange">100점 도전 중</span></div>{question && <div className="problem-card"><div className="problem-meta"><span>{question.grade}학년 · {question.semester}학기 · {question.unit}</span><span>{question.lesson} · {question.questionNumber}번</span></div><p>{question.questionText}</p></div>}<div className="challenge-card panel"><div className="complete-mark">!</div><p className="eyebrow">OFFLINE CHALLENGE</p><h1>100점 도전 중!</h1><p>백점 도우미와 대화하며 알아낸 방법으로 수학익힘책의 문제를 직접 다시 풀어 보세요.</p><p>친구에게 재채점을 받아 진짜로 해결했는지 확인한 뒤, 아래 버튼을 눌러 완료로 기록해 주세요.</p><div className="study-actions challenge-actions"><button className="button primary" onClick={onComplete}>100점 완성</button><button className="button primary" onClick={onNotebook}>오답노트로 돌아가기</button><button className="button primary" onClick={onTeacherHelp}>선생님께 도움 요청하기</button></div></div></section></main>;
}
...
function TeacherViewWithProgress({ dashboard, notice, onReset, onLogout }: { dashboard: TeacherDashboard; notice: string; onReset: (studentNumber: number) => void; onLogout: () => void }) {
  const [semester, setSemester] = useState("");
  const [unit, setUnit] = useState("");
  const units = useMemo(() => [...new Set(dashboard.lessonCatalog.filter((item) => String(item.semester) === semester).map((item) => item.unit))], [dashboard.lessonCatalog, semester]);
  const lessons = useMemo(() => dashboard.lessonCatalog.filter((item) => String(item.semester) === semester && item.unit === unit).map((item) => item.lesson), [dashboard.lessonCatalog, semester, unit]);

  return <main className="app-shell teacher-shell">
    <header className="student-header"><div className="brand-inline"><span className="brand-mark">100</span><strong>백점수익 <em>교사</em></strong></div><div className="student-header-info"><span>김선생님</span><button onClick={onLogout}>로그아웃</button></div></header>
    <section className="teacher-content">
      <div className="page-heading"><p className="eyebrow">CLASSROOM DASHBOARD · 6학년 2반</p><h1>우리 반의<br /><span>완성 현황</span></h1><p>학기와 단원을 선택해 학생별 차시 완료 여부를 확인해 주세요.</p></div>
      {notice && <div className="notice info">{notice}</div>}
      <div className="teacher-stat-grid"><StatCard label="전체 학생" value={dashboard.totalStudents} tone="blue" /><StatCard label="오늘 접속" value={dashboard.todayLoginCount} tone="blue" /><StatCard label="100점 완료 학생" value={dashboard.completedStudentCount} tone="green" /><StatCard label="교사 도움 필요" value={dashboard.teacherHelpCount} tone="red" /></div>

      <section className="teacher-progress-panel panel">
        <div className="panel-title"><div><p className="eyebrow">CLASS PROGRESS CHECK</p><h2>학생별 차시 완성 확인</h2></div><span className="muted">학생 {dashboard.totalStudents}명</span></div>
        <p className="teacher-progress-help">학기와 단원을 선택하면 학생은 세로로, 차시는 가로로 표시됩니다. 초록색 체크는 학생이 100점 완료로 표시한 차시입니다.</p>
        <div className="teacher-progress-filters"><label>학기<select value={semester} onChange={(event) => { setSemester(event.target.value); setUnit(""); }}><option value="">학기를 선택하세요</option><option value="1">1학기</option><option value="2">2학기</option></select></label><label>단원<select value={unit} onChange={(event) => setUnit(event.target.value)} disabled={!semester}><option value="">단원을 선택하세요</option>{units.map((item) => <option key={item} value={item}>{item}</option>)}</select></label></div>
        {semester && unit && lessons.length > 0 ? <div className="teacher-progress-table-wrap"><table className="teacher-progress-table"><thead><tr><th scope="col">학생</th>{lessons.map((lesson, index) => <th scope="col" key={lesson} title={lesson}>{index === 0 ? "단원도입" : `${index}차시`}</th>)}</tr></thead><tbody>{dashboard.students.map(({ student }) => <tr key={student.id}><th scope="row"><strong>{student.studentNumber}번</strong><small>{student.name}</small></th>{lessons.map((lesson) => { const completed = Boolean(dashboard.lessonCompletionsByStudent[student.id]?.[`${semester}|${unit}|${lesson}`]); return <td key={lesson}><span className={`teacher-check ${completed ? "is-complete" : ""}`} aria-label={completed ? "100점 완료" : "아직 완료하지 않음"}>{completed ? "✓" : "—"}</span></td>; })}</tr>)}</tbody></table></div> : <div className="teacher-progress-empty">학기와 단원을 모두 선택하면 학생별 차시 완료표가 보여요.</div>}
      </section>

      <div className="teacher-panel panel"><div className="panel-title"><div><p className="eyebrow">STUDENT OVERVIEW</p><h2>학생별 최근 현황</h2></div><span className="muted">총 {dashboard.totalStudents}명</span></div><div className="teacher-table-wrap"><table><thead><tr><th>번호</th><th>최근 접속</th><th>최근 문항</th><th>오류 유형</th><th>상태</th><th>완료</th><th>관리</th></tr></thead><tbody>{dashboard.students.map(({ student, currentRecord, completedCount, unresolvedCount, latestError }) => <tr key={student.id}><td><strong>{student.studentNumber}번</strong></td><td>{formatDate(student.lastLoginAt)}</td><td>{currentRecord ? "학습 진행 중" : "-"}</td><td>{latestError ?? "-"}</td><td><span className={`status-badge ${currentRecord ? statusClass[currentRecord.status] : "status-gray"}`}>{currentRecord ? statusLabels[currentRecord.status] : "시작 전"}</span></td><td>{completedCount} / {completedCount + unresolvedCount}</td><td><button className="small-button" onClick={() => onReset(student.studentNumber)}>비밀번호 000 초기화</button></td></tr>)}</tbody></table></div></div>
      <div className="teacher-lower-grid"><div className="panel insight-panel"><p className="eyebrow">COMMON CONCEPTS</p><h2>이번 주 살펴볼 개념</h2><p>학생들이 문제를 풀며 선택한 오류 유형을 모아 다음 수업에 활용할 수 있어요.</p><div className="insight-bar"><span>통분 개념 부족</span><b style={{ width: "68%" }} /></div><div className="insight-bar"><span>최소공배수 이해 부족</span><b style={{ width: "42%" }} /></div></div><div className="panel insight-panel alert-panel"><p className="eyebrow">NEEDS ATTENTION</p><h2>도움 요청 알림</h2><p>빨간색 상태의 학생에게 먼저 말을 걸어 주세요.</p><strong className="alert-number">{dashboard.teacherHelpCount}<small>명</small></strong></div></div>
    </section>
  </main>;
}
